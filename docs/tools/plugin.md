...
- Memory (Mem0) — bundled Mem0-backed memory plugin (chat-history auto-capture + auto-recall; set `plugins.slots.memory = "memory-mem0"`)
  - Includes optional per-run token telemetry (`mem0 telemetry`) for injected vs non-injected recall runs.
